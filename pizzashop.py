...
def displayMenu():
    ...
    base_price = sum(menu['Cheese']['ingredients'].values()) # the base price of the pizza takes the summation of 
    ...
    for pizza, details in menu.items():
        ...
        ingredients_list = base_ingredients + ", ".join(details["ingredients"].keys()) if pizza != "Cheese" else base_ingredients
        
        print(f"Ingredients: {ingredients_list}")
        total = base_price + sum(details["ingredients"].values())  if pizza != "Cheese" else base_price
        order_num +=1
        print(total)
...
displayMenu() 
order_number = int(input('What Would You like to order? Input 1 for Cheese, 2 for Pepperoni, 3 for Veggie '))           
order(order_number)
